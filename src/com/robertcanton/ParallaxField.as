...
package com.robertcanton
{
	...
	{
		// settings & vars
		private var xAcceleration:Number = .1 * GV.xFactor;
		private var yAcceleration:Number = .05 * GV.yFactor;
		 ...
		 */
		public function createField($container:MovieClip, $x:int, $y:int, $width:int, $height:int, $numberOfStars:int, $speedX:Number, $speedY:Number):void
		{
			trace("createField");
		...
		{			
			...
			for (i = 0; i < newNumberOfStars; i++) 
			{
				...
				// add to display list
				tempStar.scaleX *= GV.xFactor;
				tempStar.scaleY *= GV.xFactor;
